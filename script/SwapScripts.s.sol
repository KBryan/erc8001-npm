...
contract ProposeSwap is Script, SwapConstants {
    ...
    function _buildIntent(
        ...
        address counterparty,
        bytes32 payloadHash
    ) internal view returns (IAtomicSwap.AgentIntent memory intent) {
        address[] memory participants = new address[](2);
        participants[0] = proposer;
        participants[1] = counterparty;
    ...
    function _signIntent(uint256 pk, IAtomicSwap.AgentIntent memory intent) internal view returns (bytes memory) {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", swap.DOMAIN_SEPARATOR(), structHash));
...
contract AcceptSwap is Script, SwapConstants {
    ...
    function _signAcceptance(uint256 pk, IAtomicSwap.AcceptanceAttestation memory att) internal view returns (bytes memory) {
        bytes32 structHash = keccak256(abi.encode(
            ACCEPTANCE_TYPEHASH,
            att.intentHash,
            att.expiry,
...
contract ExecuteSwap is Script, SwapConstants {
    ...
    function run() external {
        ...
        // Build payload
        IAtomicSwap.CoordinationPayload memory payload = _buildPayload(proposer, counterparty, usdcAmount, wethAmount);
